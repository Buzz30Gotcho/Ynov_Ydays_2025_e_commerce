    ...
    <footer className="bg-card pt-24 pb-12 border-t border-border relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 mb-24">
          <div className="space-y-8">
            ...
            <div className="space-y-4">
              <p className="text-sm md:text-base text-text-medium font-light italic leading-relaxed">
                "L'élégance n'est pas de se faire remarquer, <br />mais de se faire remarquer."
              </p>
              <p className="text-[14px] md:text-sm text-text-light/60 font-medium leading-relaxed uppercase tracking-[0.2em]">
          ...
          <div className="space-y-8">
            <h4 className="text-[13px] md:text-xs font-black uppercase tracking-[0.4em] text-text-dark">Explore</h4>
            <ul className="space-y-4">
              {['Boutiques', 'Catalogue', 'Nouveautés'].map((item) => (
                <li key={item}>
                  <Link to={item === 'Boutiques' ? '/shops' : '/catalogue'} className="text-[14px] md:text-sm text-text-light uppercase tracking-[0.2em] font-bold hover:text-green transition-colors">
                    {item}
                  </Link>
                </li>
              ))}
          ...
          <div className="space-y-8">
            <h4 className="text-[13px] md:text-xs font-black uppercase tracking-[0.4em] text-text-dark">Partenaires</h4>
            <ul className="space-y-4">
              {['Devenir Partenaire', 'Accès Créateur', 'Espace Coursier'].map((item) => (
                <li key={item}>
                  <Link 
                    ...
                    className="text-[14px] md:text-sm text-text-dark dark:text-white uppercase tracking-[0.2em] font-bold hover:text-green transition-colors"
                  >
                    {item}
                  </Link>
                </li>
          ...
          <div className="space-y-8">
            <h4 className="text-[13px] md:text-xs font-black uppercase tracking-[0.4em] text-text-dark">Contact</h4>
            <div className="space-y-4">
              <a 
                href="https://github.com/Buzz30Gotcho?tab=repositories" 
                target="_blank" 
                rel="noopener noreferrer"
                className="block text-[14px] md:text-sm text-text-medium uppercase tracking-widest font-bold hover:text-green transition-colors"
              >
                Frédéric Sar
              </a>
            </div>
        ...
        <div className="pt-12 border-t border-border/50 flex flex-col md:flex-row justify-between items-center gap-8">
          <p className="text-[13px] md:text-xs text-text-light uppercase tracking-[0.4em] font-black">
            © {new Date().getFullYear()} Shop In Line International.
          </p>
          <div className="flex gap-12 text-[13px] md:text-xs text-text-light uppercase tracking-[0.4em] font-black">
